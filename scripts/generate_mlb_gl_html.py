"""
generate_mlb_gl_html.py — MLB game lines HTML dashboard using saved OddsAPI data.
...
Usage:
    python scripts/generate_mlb_gl_html.py
"""
...
def load_team_logos() -> dict:
    """Load MLB team logos from data directory."""
...
def fetch_mlb_espn_win_probs() -> dict:
    """Returns dict of {(away_abbrev, home_abbrev): (away_wp, home_wp)} for today's MLB games."""
    try:
        scoreboard = _fetch_json(ESPN_MLB_SCOREBOARD_URL)
    except Exception:
        return {}
    ...
    for event in events:
        ...
        try:
            data = _fetch_json(ESPN_MLB_PROBS_URL.format(gid=gid))
            items = data.get("items", [])
            if not items:
                continue
            last = items[-1]  # most recent probability snapshot
            home_wp = float(last.get("homeWinPercentage", 0))
            away_wp = float(last.get("awayWinPercentage", 0))
            result[(away_abbrev, home_abbrev)] = (away_wp, home_wp)
        except Exception:
            continue
...
def load_kalshi_gl(series_prefix: str) -> dict:
    ...
    returns a nested dict keyed by event_ticker then team_suffix.  The
    home-team market gives:
    ...
    try:
        ...
        # Build result: nested dict {event_ticker: {team_suffix: market_dict}}
        result = {}
...
def match_kalshi_market(away: str, home: str, kalshi_gl: dict) -> Optional[dict]:
    ...
    away_abbrev = MLB_TO_KALSHI_ABBREV.get(away, "")
    home_abbrev = MLB_TO_KALSHI_ABBREV.get(home, "")
    if not away_abbrev or not home_abbrev:
        return None
...
def render_card(
    ...
    _card_id_counter += 1
    card_id = f"mlb_books_{_card_id_counter}"
...
def render_html(events: list, logos: dict, espn_probs: dict) -> str:
    ...
    kalshi_gl = load_kalshi_gl("KXMLBGAME")
    ...
    if not pregame and not live:
        sections_html = '<div class="empty-section">No MLB games found.</div>'
...
<head>
  ...
  <title>MLB Game Lines &middot; OddsAPI</title>
  <style>
{CSS}
  </style>
</head>
<body>
  <header>
    <h1>MLB Game Lines &middot; OddsAPI</h1>
...
def generate(output_path=None) -> Path:
    """Render MLB game lines from saved scan data and write HTML."""
    if output_path is None:
        output_path = ROOT / "data" / "mlb_gl.html"
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)

    events = load_saved_oddsapi_games("mlb")

    logos = load_team_logos()
    espn_probs = fetch_mlb_espn_win_probs()

    html = render_html(events, logos, espn_probs)

    # Atomic write: temp file → os.replace
    tmp_fd, tmp_path = tempfile.mkstemp(
        dir=output_path.parent, prefix=".mlb_gl_", suffix=".html.tmp"
    )
    try:
